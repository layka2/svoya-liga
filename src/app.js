import {PLAYERS,TEAMMATE,GUEST,STAT_LABELS,byId} from './roster.js?v=0.3.0';
import {UNLOCKS,readProgress,writeProgress,isUnlocked,nextUnlock,recordResult} from './progression.js?v=0.3.0';
...
$('help').addEventListener('click',()=>showModal(`<p class="eyebrow">ТВОЯ ПЕРВАЯ ИГРА</p><h2>Создай свой момент.</h2><ul><li>Слева — движение, справа — действия. Удобнее играть горизонтально.</li><li><b>Бросок:</b> остановись, зажми кнопку и отпусти в зелёной зоне. Защитник, движение, усталость и большая дистанция снижают точность. Даже точный выпуск не гарантирует трёшку.</li><li><b>Обман:</b> быстро коснись броска — герой покажет замах. Соперник может прыгнуть, открыв проход.</li><li><b>Рывок:</b> короткое ускорение за выносливость. Есть перерыв между рывками; остановка восстанавливает силы.</li><li><b>Пас:</b> передай напарнику или попроси мяч у него. Не отдавай через защитника. Лови передачу на свободном месте для прибавки к точности.</li><li><b>Защита:</b> блокируй во время выпуска мяча. Перехват делай рядом с соперником; неудачный отбор на мгновение замедлит тебя.</li><li><b>Супер:</b> заряжается со временем и за удачные действия. Особый приём усиливает героя, но не отменяет выбор момента.</li><li>3 минуты, 24 секунды на атаку. При ничьей — до следующего попадания. Доигранные победы открывают героев: 1 / 3 / 6 / 10 / 15.</li></ul><p>Компьютер: WASD / стрелки, пробел — бросок, E — пас, Shift — рывок, Q — супер, C — камера.</p><p class="small-note">Прогресс сохраняется в этом браузере. Начатый и брошенный матч не считается победой.</p>`));
async function launch(){
 if(!isUnlocked(selected,progress))selected='piniv';
 $('play').disabled=true;$('play').textContent='ГОТОВИМ ПЛОЩАДКУ…';
 try{if(!game){const {CourtGame}=await import('./game.js?v=0.3.0');game=new CourtGame($('court'),{onPause:pause,onEnd:finish,onError:message=>showModal('<h2>Матч на паузе</h2><p>'+message+'</p>',returnMenu)});}
...
function returnMenu(){game?.stop();$('game-screen').classList.add('hidden');$('menu').classList.remove('hidden');render();}
