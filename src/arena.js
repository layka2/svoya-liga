...
export function createArena(scene){
 ...
 for(const side of [-1,1]){
  ...
  const rim=new T.Mesh(new T.TorusGeometry(.38,.026,8,32),orange);rim.rotation.x=Math.PI/2;rim.position.set(side*8.5,3.05,0);scene.add(rim);
  for(let i=0;i<12;i++){const a=i/12*Math.PI*2;let points=[new T.Vector3(side*8.5+Math.cos(a)*.38,3.02,Math.sin(a)*.38),new T.Vector3(side*8.5+Math.cos(a+.15)*.24,2.57,Math.sin(a+.15)*.24)];scene.add(new T.Line(new T.BufferGeometry().setFromPoints(points),new T.LineBasicMaterial({color:0xe4ddc4,transparent:true,opacity:.65})));}
  for(const y of [2.74,2.9]){const net=new T.Mesh(new T.TorusGeometry(y===2.9?.33:.275,.008,4,24),cream);net.rotation.x=Math.PI/2;net.position.set(side*8.5,y,0);scene.add(net);}
 }
