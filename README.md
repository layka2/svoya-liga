...
## Обновление 0.5 — Король площадки

Новый основной режим — серия из трёх коротких матчей до 7 очков или 90 секунд. При равном счёте игра продолжается до следующего попадания. После первых двух побед выбираешь одно из трёх тактических усилений; оно действует у твоей команды до конца серии. После поражения начинаешь новый забег.

- Три лиги: **Двор** доступен сразу, **Лицей** открывается за один титул, **Легенды** — за три. Каждый уровень предлагает более сильные команды.
- Шесть усилений: бросок после паса, рывок после финта, точный бросок после остановки, второй шанс после подбора, награда за успешную защиту и восстановление выносливости. Они помогают выбранному стилю, но не гарантируют попадание.
- **18 трофеев:** бронза, серебро и золото для каждого из шести героев. Повторный титул увеличивает счётчик защит; лучший титул отображается на карточке и в матче.
- Серия сохраняется между раундами, включая предложенные усиления. После выхода незавершённый матч начинается заново. Обычный трёхминутный матч доступен отдельной кнопкой.
- **Заслон и открывание:** кнопка «ЗАСЛОН» / G зовёт напарника к защитнику. Он занимает стойку, помогает освободить пространство при реальном контакте и открывается под пас к кольцу. У команды есть время восстановления розыгрыша. Соперники тоже могут использовать эту комбинацию.

Подробности: [правила серии и усилений](docs/crown-v05.md). Все доигранные победы в обоих режимах участвуют в открытии героев.

![Король площадки, версия 0.5](docs/crown-v05.png)
...
- Заслон: с мячом подойти к защитнику, позвать напарника, обойти его после остановки и искать передачу к кольцу.
...
- Компьютер: WASD/стрелки, пробел — бросок, E — пас/перехват, Q — супер, Shift — рывок, F — финт, G — заслон, C — камера, R — поворот, Esc — пауза.
...
- Обычный матч — 3 минуты; матч серии — до 7 очков или 90 секунд. В обоих режимах 24 секунды на атаку, два кольца, очки 2/3; ничья разрешается следующим попаданием. Штрафные, фолы и пробежки пока не реализованы.
...
- `src/league.js` — три раунда, усиления, дивизионы, трофеи и сохранения серии.
...
- `tests/logic.mjs` — проверка игровой логики; вместе с `tests/league.mjs` запускается командой `npm test` при установленном Node.js.
